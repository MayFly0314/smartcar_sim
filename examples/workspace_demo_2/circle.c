...
static int left_contactpoint_y=0;//右圆环出环时左边界局部极大值点行号
static int right_contactpoint_y=0;//左圆环出环时右边界局部极小值点行号
...
/*
 *@brief 在右侧拐点和接触点之间重构右边界，并更新对应中线
 *@param img 当前帧二值图像
 *@param corner_x 右侧拐点横坐标
 *@param corner_y 右侧拐点行号
 *@param contact_x 接触点横坐标
 *@param contact_y 接触点行号
 *@return 无
 */
static void rebuild_right_boundary(uint8_t img[IMG_H][IMG_W],
                                   int corner_x,int corner_y,
                                   int contact_x,int contact_y)
{
    int start_y;
...
void identify_circle_state(void)
{
    ...
    switch(circle_context.state)
    {
        case CIRCLE_ENTRY_DOWN:
            ...
            {
                int contact_y=find_right_contactpoint();

                //右下拐点位于右切点下方时，当前下拐点阶段仍然成立
                if(corner_list[2].type==CORNER_R_DOWN&&
                   contact_y>0&&corner_list[2].y>contact_y)
                    break;
        ...
        case CIRCLE_ENTRY_UP_EXTEND:
                    ...
                    corner_list[3].y>=CIRCLE_ENTRY_UP_ROW_LIMIT)
            {
                ...
                if(contact_y>0&&corner_list[3].y<contact_y)
                    circle_context.state=CIRCLE_ENTRY_LINK;
            }
            break;
        ...
        case CIRCLE_INSIDE:
            ...
            {
                left_contactpoint_y=find_left_contactpoint();
                if(left_contactpoint_y>0)
                    circle_context.state=CIRCLE_EXIT_LINK;
            }
            break;
    ...
    switch(circle_context.state)
    {
        ...
        case CIRCLE_ENTRY_DOWN:
                    ...
                    corner_list[2].type==CORNER_R_DOWN)
            {
                ...
                if(contact_y>0&&corner_list[2].y>contact_y)
                {
                    rebuild_right_boundary(img,
                                           corner_list[2].x,
                                           corner_list[2].y,
                                           right_boundary[contact_y],
                                           contact_y);
                }
            }
            break;
        ...
        case CIRCLE_ENTRY_UP_EXTEND:
                    ...
                    corner_list[3].y<CIRCLE_ENTRY_UP_ROW_LIMIT)
            {
                ...
                if(contact_y>=3&&contact_y<=IMG_H-4&&
                   corner_list[3].y<contact_y&&
                   left_boundary_valid[contact_y-3]&&
                   left_boundary_valid[contact_y]&&
                   left_boundary_valid[contact_y+3])
                {
                    left_dx=left_boundary[contact_y+3]-
                            left_boundary[contact_y-3];
                    x_bottom=corner_list[3].x-
                             left_dx*(IMG_H-1-corner_list[3].y)/6;

                    linktwo(img,
                            corner_list[3].x,corner_list[3].y,
                            x_bottom,IMG_H-1);
                    for(row=corner_list[3].y;row<IMG_H;row++)
                    {
                        right_boundary[row]=corner_list[3].x-
                            left_dx*(row-corner_list[3].y)/6;
                        center[row]=(left_boundary[row]+right_boundary[row])/2;
                    }
                }
            }
        ...
        case CIRCLE_ENTRY_LINK:
                    ...
                    corner_list[3].y>=CIRCLE_ENTRY_UP_ROW_LIMIT)
            {
                ...
                if(contact_y>0&&corner_list[3].y<contact_y&&
                   left_boundary_valid[contact_y])
                {
                    contact_x=left_boundary[contact_y];
                    circle_context.saved_link_dx=contact_x-corner_list[3].x;
                    circle_context.saved_link_dy=contact_y-corner_list[3].y;
                    circle_context.saved_link_slope_valid=1;
                    linktwo(img,
                            corner_list[3].x,corner_list[3].y,
                            contact_x,contact_y);
                    for(row=corner_list[3].y;row<=contact_y;row++)
                    {
                        left_boundary[row]=corner_list[3].x+
                            (contact_x-corner_list[3].x)*
                            (row-corner_list[3].y)/
                            (contact_y-corner_list[3].y);
                        center[row]=(left_boundary[row]+right_boundary[row])/2;
                    }
                }
            }
        ...
        case CIRCLE_ENTRY_SLOPE_EXTEND:
            //沿ENTRY_LINK最后有效斜率延长，再在入环侧区域重新扫线
            if(circle_context.direction==CIRCLE_DIR_LEFT&&
               corner_list[1].type==CORNER_L_UP&&
               circle_context.saved_link_slope_valid&&
               circle_context.saved_link_dy>0)
            {
                ...
                scan_lines(img);
                    ...
                    circle_context.saved_link_dy>0)
            {
                int up_x=corner_list[3].x;
                int up_y=corner_list[3].y;
                int x_bottom=up_x+
                    (int)((long)circle_context.saved_link_dx*
                          (IMG_H-1-up_y)/circle_context.saved_link_dy);

                linktwo(img,up_x,up_y,x_bottom,IMG_H-1);
                find_longest_whiteline_range(img,up_x,IMG_W-1);
                scan_lines(img);
            }
            break;

        case CIRCLE_INSIDE:
            //底行出现多个有效白色连通段时，选择环岛方向一侧的区域重新扫线
            if(circle_context.direction==CIRCLE_DIR_LEFT)
            {
                int x;
                int is_white;
            ...
            else if(circle_context.direction==CIRCLE_DIR_RIGHT)
            {
                ...
                for(x=0;x<=IMG_W;x++)
                {
                    ...
                    else if(!is_white&&run_start>=0)
                    {
                        ...
                        {
                            valid_run_count++;
                            right_run_start=run_start;
                            right_run_end=run_end;
                        }
                        run_start=-1;
                    }
                }
        ...
        case CIRCLE_EXIT_LINK:
                    ...
                    left_contactpoint_y>0)
            {
                int row;
                int roi_top=IMG_H-max_length;
                int right_invalid_y=-1;
                int left_point_x=left_boundary[left_contactpoint_y];

                if(roi_top<0)roi_top=0;
                for(row=IMG_H-1;row>=roi_top;row--)
                {
                    if(!right_boundary_valid[row])
                        right_invalid_y=row;
                }

                if(right_invalid_y>=0)
                {
                    if(left_contactpoint_y>right_invalid_y&&
                       right_boundary[right_invalid_y]>left_point_x)
                    {
                        circle_context.saved_exit_dx=
                            right_boundary[right_invalid_y]-left_point_x;
                        circle_context.saved_exit_dy=
                            left_contactpoint_y-right_invalid_y;
                        circle_context.saved_exit_slope_valid=1;
                    }
                    linktwo(img,
                            left_point_x,left_contactpoint_y,
                            right_boundary[right_invalid_y],right_invalid_y);
                    find_longest_whiteline(img);
                    scan_lines(img);
                }
            }
        ...
        case CIRCLE_EXIT_SLOPE_EXTEND:
            //使用EXIT_LINK最后有效斜率，从最底行边界点向出环方向的屏幕边缘延长
            if(circle_context.direction==CIRCLE_DIR_LEFT&&
               circle_context.saved_exit_slope_valid&&
               circle_context.saved_exit_dx>0&&
               circle_context.saved_exit_dy>0)
            ...
            {
                int start_y=IMG_H-1;
                int start_x=left_boundary[IMG_H-1];
                int end_y=start_y-
                    (int)((long)(IMG_W-1-start_x)*
                          circle_context.saved_exit_dy/
                          circle_context.saved_exit_dx);

                if(end_y<0)end_y=0;
                if(end_y>start_y)end_y=start_y;
                linktwo(img,start_x,start_y,IMG_W-1,end_y);
                find_longest_whiteline(img);
                scan_lines(img);
            }
            break;
        ...
        case CIRCLE_EXIT_EXTEND:
            ...
            {
                int row;
                int center_start=corner_list[3].y-4;

                if(center_start<0)center_start=0;
                right_lengthen(img,corner_list[3].x,corner_list[3].y);
                for(row=center_start;row<IMG_H;row++)
                    center[row]=(left_boundary[row]+right_boundary[row])/2;
            }
            break;
