...
4. **看结果**：右侧图像区滚轮缩放、拖拽平移；缩放 ≥8 倍显示像素网格；状态栏显示光标下像素坐标和灰度值。"叠加"开关控制绘图显示，"处理后"开关切换原图/算法写回的图。代码里 `sim_plot("name", value)` 记录的变量会出现在图像下方的**监视面板**：变量按多列紧凑排列，仅显示名称和当前帧值；双击任意变量打开独立的大曲线窗口，曲线内可点击或拖动跳帧。点"监视"标题可折叠。`sim_tag(x, y, "文本", ...)` 给图上某点附加说明（不遮挡图像）：鼠标悬停该处弹出查看，或看图像下方**"本帧标注"列表**（点击行图上高亮该点）。
...
- **数据记录方案**（菜单【文件 → 数据记录方案（压缩存图/参数）】，独立设计器窗口）：可视化拼出每帧的 SD 存储布局——帧头 hex + 任意参数（uint8/int16/int32/float 等，填车端变量名）+ 图像（**压缩二值 1bit/像素**（13020B→1628B，写卡阻塞降约 6 倍）/ 原始 8bit / 不存图）。
  ...
  - 「按此方案打开记录文件」自动解压图像进时间轴，参数在独立的**车端记录**面板中按多列显示（名称可在设计器里随时改）；双击参数可查看完整曲线；
...
sim_draw_text(x, y, c, "th=%d", th);    // 文本（锚定图像坐标 + 左上角大字状态板）
sim_log("otsu = %d", th);               // 控制台日志（自动带帧号）
sim_plot("error", err);                 // 数值监视（面板只显示变量名和当前值）
                                        // 双击变量 → 单独的曲线窗口，Y 轴上下限可手调
...
> 界面壁纸：菜单【界面 → 选择壁纸】选本地图片（暗度可调、自动记忆）；
> 编辑器与终端为浏览器内核不透壁纸，图像区保持纯色底。
