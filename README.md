...
- **SD 卡直读**（菜单【文件 → 从 SD 卡直接读取录制】）：**插上读卡器直接读扇区，不用 HxD 导出**。
  - 选那张可移动盘 → 点「扫描录制」→ 按当前记录方案的帧头在卡上定位录制段，**自动算出起始扇区和帧数**（不用你算）→ 列表里选一段看首帧预览 → 「载入选中的录制」进时间轴。
  - 扫不到时可手填起始 LBA，点「按此 LBA 读取」（帧数仍自动探测）。
  - **只读**：不会向 SD 卡写入任何数据。只列可移动盘，系统盘在 Windows 层面就被挡住。免管理员权限。
  - ⚠ 卡上没有 Windows 认得的文件系统时会弹「需要格式化磁盘」，**务必点取消**——格式化会清空录制。
  - 解出来是花屏/全黑 → 多半是记录方案与车端录制时不一致（帧头/参数/分辨率），预览里会提示。
...
- **数据记录方案**（菜单【文件 → 数据记录方案（压缩存图/参数）】，独立设计器窗口）：可视化拼出每帧的 SD 存储布局——帧头 hex + 任意参数（uint8/int16/int32/float 等，填车端变量名）+ 图像（**压缩二值 1bit/像素**（13020B→1628B，写卡阻塞降约 6 倍）/ 原始 8bit / 不存图）。
  ...
  - 布局摘要会显示**每帧余量**（补零到整扇区还剩多少字节）。参数越加越多，一旦吃光余量就会多占一个扇区、写卡时间按比例变长——摘要里会直接告诉你「还能再加约 N 个参数」。
  - 生成的代码用方案自己的 `REC_IMG_W/REC_IMG_H` 跑循环，并带一条 `_Static_assert` 与工程的 `IMG_W/IMG_H` 对表：**分辨率不一致会在编译期报错**，而不是到车上写越界踩内存。（工程若没有这两个宏，删掉那一行即可。）
...
> 串口/蓝牙实时图传、SD 卡读图（含插卡直读扇区）已实现，见上文「数据来源」。
